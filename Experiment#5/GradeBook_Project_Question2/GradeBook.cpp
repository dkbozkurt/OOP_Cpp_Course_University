...
GradeBook::GradeBook(string name,const int gradesArray[][ii])		//Constructor
{
	setCourseName(name);
	A=0;
	B=0;
	...
	for ( int grade = 0; grade < students; grade++ ){
		for(int j=0; j<ii;j++)
		grades[grade][j]=gradesArray[grade][j];
	}
}
...
int GradeBook::getMinimum(void){
	...
	for(int grade=0;grade<students;grade++){
		for(int j=0;j<ii;j++){
			if (grades[grade][j] < lgrade)
				lgrade=grades[grade][j];	//new lowest grade ll be changed.
		}
	}
	return lgrade;
}
...
int GradeBook::getMaximum(void){
	...
	for(int grade=0;grade<students;grade++){
		for(int j=0;j<ii;j++){
			if (grades[grade][j] > hgrade)
				hgrade=grades[grade][j];	//new lowest grade ll be changed.
		}
	}	
	return hgrade;
}

double GradeBook::getAverage(const int setOfGrades[],const int grades){	//For experiment 5
...
void GradeBook::outputBarChart(void){
	...
	for ( int grade = 0; grade < students; grade++ ){
		for(int j=0;j<ii;j++){	
			++frequency[ grades[ grade ][j] / students ];		///
	}
}
...
void GradeBook::outputGrades(void){
	...
	for(int grade=0; grade < students ; grade++){
		...
		cout << "Student " << setw( 2 ) << grade + 1;
		for(int j=0;j<ii;j++)
			cout << setw( 8 ) << grades[grade][j];
			
			
	double average = getAverage( grades[ grade ], ii );
